 ...
 */
export async function getMyProfile(config: MentoraAPIConfig): Promise<APIResult<UserProfile>> {
	...
	return tryCatch(async () => {
		...
		if (!snapshot.exists()) {
			...
			await setDoc(docRef, {
				...
				photoURL: profile.photoURL !== undefined ? profile.photoURL : currentUser.photoURL,
				createdAt: now,
				updatedAt: now
			});
		} else {
