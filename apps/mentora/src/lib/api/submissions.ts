...
export async function getSubmission(
    ...
    return tryCatch(async () => {
        const docRef = doc(
            config.db,
            AssignmentSubmissions.docPath(assignmentId, userId),
        );
...
export async function listAssignmentSubmissions(
    ...
    return tryCatch(async () => {
        ...
        const q = query(
            collection(
                config.db,
                AssignmentSubmissions.collectionPath(assignmentId),
            ),
            ...constraints,
        );
...
export async function startSubmission(
    ...
    return tryCatch(async () => {
        const docRef = doc(
            config.db,
            AssignmentSubmissions.docPath(assignmentId, currentUser.uid),
        );
