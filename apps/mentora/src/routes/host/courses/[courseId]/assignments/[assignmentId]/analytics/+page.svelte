<script lang="ts">
    ...
    let assignmentId = $derived((page.params as Record<string, string>).assignmentId);
    ...
    let sortedScores = $derived(() => {
        ...
        scores.sort((a, b) => {
            ...
            if (sortKey === "name") {
                ...
                aVal =
                    a.dimensions[
                        sortKey as keyof typeof a.dimensions
                    ];
                bVal =
                    b.dimensions[
                        sortKey as keyof typeof b.dimensions
                    ];
            }
    ...
    async function generateReport() {
        generating = true;
        try {
            const res = await api.backend.call<AssignmentAnalytics["classReport"]>(
    ...
    <div class="container mx-auto max-w-7xl px-4 py-8">
        ...
        <div class="mb-8">
            ...
            <div class="flex flex-col justify-between gap-4 md:flex-row md:items-end">
                <div>
                    <h1 class="flex items-center gap-3 font-serif text-3xl text-white">
        ...
        {:else if data}
            ...
            <div class="mb-8">
                <GlassCard>
                    <h2 class="mb-4 flex items-center gap-2 font-serif text-xl text-white">
                        <FileText class="text-brand-gold h-5 w-5" />
                        作業概覽
                    </h2>
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                        <div>
                            <div class="text-text-secondary mb-1 text-sm uppercase tracking-wider">作業名稱</div>
                            <div class="text-lg font-medium text-white">{data.title}</div>
                        </div>
                        <div>
                            <div class="text-text-secondary mb-1 text-sm uppercase tracking-wider">截止日期</div>
                            <div class="flex items-center gap-2 text-lg font-medium text-white">
                                <Calendar class="text-brand-gold h-4 w-4" />
                                {data.dueAt ? formatDate(data.dueAt) : "無截止日期"}
                            </div>
                        </div>
                        <div>
                            <div class="text-text-secondary mb-1 text-sm uppercase tracking-wider">提交狀況</div>
                            <div class="flex items-center gap-2 text-lg font-medium text-white">
                        ...
                        <div>
                            <div class="text-text-secondary mb-1 text-sm uppercase tracking-wider">班級平均分數</div>
                            <div class="text-lg font-medium text-white">
                                {#if data.avgScore !== null}
                                    <span class="text-brand-gold font-mono text-2xl font-bold">
                                        {data.avgScore.toFixed(1)}
                                    </span>
                                    <span class="text-text-secondary text-sm"> / 5.0</span>
                                {:else}
                                    <span class="text-text-secondary">尚未評估</span>
                                {/if}
                            </div>
                        </div>
                    </div>
            ...
            <div class="mb-8">
                <GlassCard className="flex flex-col">
                    <div class="mb-6">
                        <h2 class="flex items-center gap-2 font-serif text-xl text-white">
                    ...
                    {#if data.spectrum.length > 0}
                        <div class="relative h-48 p-4">
                            <!-- Spectrum axis -->
                            <div class="from-status-error via-brand-silver to-status-success absolute right-4 left-4 top-1/2 h-1 -translate-y-1/2 rounded-full bg-linear-to-r opacity-50"></div>

                            <!-- Labels -->
                            <div class="text-status-error absolute bottom-0 left-4 text-xs font-bold uppercase tracking-wider">
                                強烈反對 (-10)
                            </div>
                            <div class="text-text-secondary absolute bottom-0 left-1/2 -translate-x-1/2 text-xs font-bold uppercase tracking-wider">
                                中立
                            </div>
                            <div class="text-status-success absolute right-4 bottom-0 text-xs font-bold uppercase tracking-wider">
                                強烈支持 (+10)
                            </div>
                            ...
                            <div class="absolute inset-4 bottom-8">
                                {#each data.spectrum as point, i (i)}
                                    {@const left = ((point.stance + 10) / 20) * 100}
                                    {@const top = (spectrumSeeds[i] ?? 0.5) * 70 + 5}
                                    <div
                                    ...
                                    >
                                        {#if showDetail}
                                            <div class="pointer-events-none absolute bottom-full left-1/2 mb-2 -translate-x-1/2 rounded border border-white/20 bg-black/80 px-2 py-1 text-xs whitespace-nowrap text-white opacity-0 transition-opacity group-hover:opacity-100">
                                                {point.name}
                                            </div>
                                        {/if}
                                    </div>
                    ...
                    {:else}
                        <div class="text-text-secondary py-12 text-center text-sm">
                            尚無立場資料
                        </div>
                    {/if}
                </GlassCard>
            ...
            {#if data.excerpts.length > 0}
                <div class="mb-8">
                    <GlassCard>
                        <h2 class="mb-6 flex items-center gap-2 font-serif text-xl text-white">
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            <!-- Pro excerpts -->
                            <div>
                                <div class="text-status-success mb-3 text-sm font-bold uppercase tracking-wider">
                                    支持方觀點
                                </div>
                                <div class="space-y-3">
                                    ...
                                    {#if proExcerpts.length === 0}
                                        <p class="text-text-secondary text-sm">尚無支持方摘錄</p>
                                    {/if}
                                </div>
                            </div>
                            <!-- Con excerpts -->
                            <div>
                                <div class="text-status-error mb-3 text-sm font-bold uppercase tracking-wider">
                                    反對方觀點
                                </div>
                                <div class="space-y-3">
                                    ...
                                    {#if conExcerpts.length === 0}
                                        <p class="text-text-secondary text-sm">尚無反對方摘錄</p>
                                    {/if}
                                </div>
                            </div>
                        </div>
                ...
                <GlassCard>
                    ...
                    {#if data.classReport}
                        ...
                        {#if data.classReport.submissionCount < data.submittedCount}
                            <div class="mb-4 flex items-center gap-3 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
                                <AlertTriangle class="h-5 w-5 shrink-0 text-amber-400" />
                                <span class="text-sm text-amber-400">
                                    有 {data.submittedCount - data.classReport.submissionCount} 份新提交，報告可能已過時
                                </span>
                                <CosmicButton
                                    variant="secondary"
                    ...
                    {:else}
                        <div class="py-8 text-center">
                            ...
                            <CosmicButton
                                variant="primary"
                                onclick={generateReport}
                                disabled={generating || data.submittedCount === 0}
                            >
                                {#if generating}
                            ...
                            {#if data.submittedCount === 0}
                                <p class="text-text-secondary mt-2 text-xs">尚無提交，無法產生報告</p>
                            {/if}
                        </div>
                    {/if}
                </GlassCard>
                        ...
                        {#if data.scores.length > 0}
                            <div class="overflow-x-auto">
                                <table class="w-full text-left text-sm">
                                    <thead>
                                        <tr class="text-text-secondary border-b border-white/10 text-xs uppercase tracking-wider">
                                            {#each scoreHeaders as h (h.key)}
                                                ...
                                                >
                                                    <span class="inline-flex items-center gap-1">
                                                        {h.label}
                                                        {#if sortKey === h.key}
                                                            <ArrowUpDown class="h-3 w-3 text-brand-gold" />
                                                        {/if}
                                                    </span>
                                                </th>
                                            {/each}
                                    ...
                                    <tbody>
                                        {#each sortedScores() as score (score.userId)}
                                            <tr class="border-b border-white/5 transition-colors hover:bg-white/5">
                                                ...
                                                <td class="px-3 py-3 font-mono font-bold {score.overallScore < 2.5 ? 'text-amber-400' : 'text-white'}">
                                                    {score.overallScore.toFixed(1)}
                                                </td>
                                                <td class="px-3 py-3 font-mono {score.dimensions.argumentQuality < 2.5 ? 'text-amber-400' : 'text-text-secondary'}">
                                                    {score.dimensions.argumentQuality.toFixed(1)}
                                                </td>
                                                <td class="px-3 py-3 font-mono {score.dimensions.criticalThinking < 2.5 ? 'text-amber-400' : 'text-text-secondary'}">
                                                    {score.dimensions.criticalThinking.toFixed(1)}
                                                </td>
                                                <td class="px-3 py-3 font-mono {score.dimensions.principleExtraction < 2.5 ? 'text-amber-400' : 'text-text-secondary'}">
                                                    {score.dimensions.principleExtraction.toFixed(1)}
                                                </td>
                                                <td class="px-3 py-3 font-mono {score.dimensions.openness < 2.5 ? 'text-amber-400' : 'text-text-secondary'}">
                                                    {score.dimensions.openness.toFixed(1)}
                                                </td>
                                                <td class="px-3 py-3 font-mono {score.dimensions.coherence < 2.5 ? 'text-amber-400' : 'text-text-secondary'}">
                                                    {score.dimensions.coherence.toFixed(1)}
                                                </td>
                                                <td class="text-text-secondary px-3 py-3 text-xs">
                                                    {score.submittedAt ? formatDate(score.submittedAt) : "-"}
                                                </td>
                                                <td class="px-3 py-3">
                                                    {#if score.late}
                                                        <span class="rounded bg-red-500/20 px-2 py-0.5 text-xs text-red-400">遲交</span>
                                                    {:else}
